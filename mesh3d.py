"""Step 6/8 - minimal 3D engine: rotate, project, shade, cast a shadow.
...
class Mesh3D:
    ...
    edge_only_faces: set[int] = field(default_factory=set)


# --- Transform: rotate, then project -------------------------------------
...
def face_draw_order(rotated_vertices: np.ndarray, faces: list[tuple[int, ...]]) -> list[int]:
    ...
    depths.sort(key=lambda t: t[0], reverse=True)
